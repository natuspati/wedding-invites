...
from schemas.rsvp import RSVPCreateSchema, RSVPFilterSchema, RSVPInDBSchema
...
class RSVPRepo:
    ...
    def select(self, filters: RSVPFilterSchema) -> list[RSVPInDBSchema]:
        clauses = [RSVP.created_at >= filters.start, RSVP.created_at <= filters.end]
        if filters.name:
            clauses.append(
                or_(RSVP.name.ilike(filters.name), RSVP.partner_name.ilike(filters.name)),
            )

        stmt = select(RSVP).where(and_(*clauses))
